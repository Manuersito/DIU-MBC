...
public class NuevoArticuloController {
    ...
    // Establece el modelo del catálogo.
    // Establece el modelo del catálogo.
    public void setCatalogoModel(CatalogoModel catalogoModel) {
        ...
        double progresoInicial = Math.min(disponibles.get() / 50.0, 1.0); // Calcula el progreso inicial
        productosDisponiblesProgressBar.setProgress(progresoInicial);
        ...
        disponibles.addListener((observable, oldValue, newValue) -> {
            ...
            double progreso = Math.min(newValue.doubleValue() / 50.0, 1.0);
